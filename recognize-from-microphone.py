...
if __name__ == '__main__':
  ...
  def return_matches(hashes):
    ...
    # helper to convert DB / numpy / bytes values to int safely
    def to_int(val):
      # bytes from sqlite or numpy bytes -> try decode then fallback to int.from_bytes
      if isinstance(val, bytes):
        try:
          return int(val.decode('utf-8'))
        except Exception:
          return int.from_bytes(val, byteorder='little', signed=False)
      # other numeric types (including numpy ints) should work with int()
      return int(val)
    ...
    for split_values in grouper(values, 1000):
      ...
      for hash, sid, offset in x:
        ...
        yield (sid, to_int(offset) - to_int(mapper[hash]))
